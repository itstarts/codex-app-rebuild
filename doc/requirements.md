...
- `patch:check` 必须先验证原始 `app.asar` 的整体 SHA-256，再解包到任务结束即删除的临时快照；所有 dry-run 只能读取该快照，不依赖或修改持久化 `_asar` 的当前 patch 状态。
...
- 对已登记的上游版本/build，必须用原始 ASAR 整体哈希和五个关键角色的 AST 结构证据证明官方原生 tier 链路；模块级 SHA-256 只作为审计信息，不作为单独硬门禁。ASAR 整体哈希或结构证据变化时必须失败，且不得降级到旧扫描逻辑。
