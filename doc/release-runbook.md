...
`npm run patch:check` 会先验证持久化官方 `app.asar` 的整体 SHA-256，再在 `.cache/` 中创建一次性原始解包快照并对该快照执行全部 dry-run；因此即使持久化 `_asar` 已经执行过 patch，重复 check 也不会读取或覆盖这些修改。模块级 SHA-256 仅作为审计输出，Fast tier 的硬门禁由 ASAR 整体哈希、原始文件与检查快照的一致性及 AST 结构证据组成。
...
GitHub About 是公开入口，应与 `README.md` 的当前能力保持一致。主页固定指向 `https://github.com/itstarts/codex-app-rebuild/releases/latest`，避免写入会过期的具体 tag；描述应覆盖 macOS arm64 rebuild、Sparkle 更新和 API Key GPT-5.6 模型目录支持。推荐 topics 为现有集合加上 `arm64`、`gpt-5` 和 `openai-api`。

更新后回读仓库元数据，确认 description、homepage 和 topics 均已生效：

```bash
gh repo view itstarts/codex-app-rebuild \
